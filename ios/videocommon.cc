...
static const int trueResolutionsCount = 15;

static const int trueResolutionsList[][trueResolutionsCount] = {
  {128, 72},
  {256, 144},
  {385, 216},
  {512, 288},
...
void ComputeCrop(int cropped_format_width, int cropped_format_height,
  ...
  double frame_aspect = static_cast<double>(frame_height * pixel_height) /
    static_cast<double>(frame_width * pixel_width);
  //cropping video to 16:9 format.
  bool isCurrentResolutionTrue = false;
  for (int i = 0; i < trueResolutionsCount; ++i) {
    isCurrentResolutionTrue = (frame_width == trueResolutionsList[i][0] && frame_height == trueResolutionsList[i][1]) ||
    (frame_width == trueResolutionsList[i][1] && frame_height == trueResolutionsList[i][0]);
  }
  double crop_aspect = 16 / 9.0; // default
  if (cropped_format_width != 0 && cropped_format_height != 0)
    crop_aspect = cropped_format_width / static_cast<double>(cropped_format_height);
    
  float epsilon = 0.001;
  if(!isCurrentResolutionTrue || std::abs(crop_aspect - frame_aspect) > epsilon){
    // if aspect ratio is 16/9 we have to select appropriate resolution
    // from trueResolutionsList
    if (std::abs(crop_aspect - 16 / 9.0) < epsilon) {
      for (int i = trueResolutionsCount; i > 0; --i) {
        if (frame_width >= trueResolutionsList[i][0] && frame_height >= trueResolutionsList[i][1]) {
          frame_width = trueResolutionsList[i][0];
          frame_height = trueResolutionsList[i][1];
          break;
        }
      }
    }
    //for another aspect ratios just compute values proportionally
    else {
      if (frame_width > frame_height) {
        frame_width = frame_height;
        frame_height = static_cast<int>((frame_width * pixel_height)
          / (crop_aspect * pixel_width) + 0.5f) & ~1;
      } else {
