  ...
  *format_out = format_in;
  ...
  if (constraint.key == MediaConstraintsInterface::kMinWidth) {
    ...
    return  (value >= ratio - kRoundingTruncation);
      ...
      constraint.key == MediaConstraintsInterface::kCropRatioWidth ||
      constraint.key == MediaConstraintsInterface::kCropRatioHeight) {
...
void SetCropAsectRatio(const MediaConstraintsInterface::Constraints& constraints,
    int* ratio_w,
    int* ratio_h) {
  typedef MediaConstraintsInterface::Constraints::const_iterator
        ConstraintsIterator;
  for (ConstraintsIterator constraints_it = constraints.begin();
         constraints_it != constraints.end(); ++constraints_it) {
    if (constraints_it->key == MediaConstraintsInterface::kCropRatioWidth) {
      *ratio_w = rtc::FromString<int>(constraints_it->value);
      continue;
    } else if (constraints_it->key == MediaConstraintsInterface::kCropRatioHeight) {
      *ratio_h = rtc::FromString<int>(constraints_it->value);
      continue;
    }
  }
}
...
void VideoSource::Initialize(
  ...
  if (constraints) {
    ...
    int ratio_w = 0;
    int ratio_h = 0;
    SetCropAsectRatio(optional_constraints, &ratio_w, &ratio_h);
    video_capturer_->UpdateAspectRatio(ratio_w, ratio_h);
  }
